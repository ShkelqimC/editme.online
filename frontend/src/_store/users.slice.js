...
function createExtraActions() {
  ...
  function update() {
    return createAsyncThunk(`${name}/update`, async function ({ id, data }, { getState, dispatch }) {
    //   debugger;
      const result = await fetchWrapper.put(`${baseUrl}/${id}`, data);
      console.log(result, "result");
      // update stored user if the logged in user updated their own record
      const auth = getState().auth?.jwtToken;
